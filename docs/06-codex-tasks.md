...
- `POST /api/books/{bookId}/chat` 단발성 대화 API 구현
- 활성 캐릭터 프로필, 질문별 RAG 원문과 캐릭터의 직접 KG 관계를 프롬프트로 조립
- 답변, 근거 충분 여부, 사용 문단·관계 ID의 구조화 응답 및 서버 검증
- 근거 부족 시 서버 고정 모름 응답 반환
- 사용 근거 ID, RAG 범위와 직접 KG 관계를 발표용 디버그 정보로 반환
- 정상 응답 후 `CHAT_READY` 상태 전환
- Fake 기반 통합 테스트와 `alice-demo` 실제 OpenAI 대화 검증 완료
- 초기 PoC에서는 대화 이력을 저장하지 않음
...
- 별도 Python·Gradio 파이프라인 QA 워크벤치(`qa`, 포트 7860) 추가
- 도서 가져오기, 단계별·검수 전 일괄 실행, 원본 JSON과 소요 시간·오류 로그 표시
- 후보별 역할·대화 가능 여부 선택과 승인·거절 검수 기능
- 활성 캐릭터 승인 후 프로필·RAG 마무리 실행 및 전체 debug 대화 테스트 제공
- Spring Boot REST API만 호출하도록 구성하고 기존 Next.js `/qa` 화면은 제거
...
## Task 13. 채팅방 단기 대화 메모리

상태: DONE

구현 결과:

- 채팅 요청에 최근 USER/ASSISTANT 메시지를 선택적으로 전달하는 API 계약 추가
- 기록이 없는 첫 질문은 기존 단일 질문 동작을 유지하고, 기록이 있을 때 독립 질문 재작성 LLM 호출
- 명확한 독립 질문으로 RAG 검색 후 원래 질문에 답하도록 최종 프롬프트 구성
- 모호한 대상은 후보 유무에 따라 캐릭터 말투로 되묻고 RAG 검색 생략
- 재작성 실패 시 원래 질문을 사용하는 기존 경로로 복구하고 debug에 fallback 표시
- 이전 답변은 대화 참조 해석에만 사용하고 RAG 문단과 KG 관계만 사실 근거로 검증
- `ANSWER`, `CLARIFICATION`, `UNKNOWN` 응답 유형 구분
- Next.js 화면에 누적 말풍선, 응답 생성 상태, `새 대화` 버튼, 책 변경 초기화 구현
- 최근 3턴과 메시지 길이 제한 적용 및 Fake AI 기반 통합 테스트 추가

---

## Task 14. 질문 의도 판정과 일상 대화

상태: DONE

구현 결과:

- 모든 입력에서 기존 질문 재작성 LLM이 `FACTUAL` 또는 `SOCIAL` 의도를 함께 판정
- `FACTUAL`은 독립 질문, 모호성, RAG·KG 검색과 근거 ID 검증을 기존대로 수행
- `SOCIAL`은 RAG·KG를 생략하고 캐릭터 말투로 인사·감사·사과·작별·가벼운 반응 생성
- 캐릭터 관련 사실 질문이 일상 대화로 우회되지 않도록 의도 경계 프롬프트 추가
